...
export const setPills =async (pills)=>{
  try {
    const value = getData('pillsList');// 여기서 pills 목록을 불러옴
    // value.push(pills)
    // pills.map((name) => itemName=name.itemName)
    value.push(...pills)
    storeData('pillsList',value)

  } catch (e) {
    console.log("setPills err");
  }
  console.log("Done");
}

export const getAllKeys = async () => { //Asyncstorage의 key들의 배열을 리턴
  let keys = []
  try {
